#!/usr/bin/env bash
# Build M1.6v3 ISO, verify boot, and upload to GitHub release v8-milestone1.6v3.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ISO="$ROOT/vm/backroot8-live.iso"
ASSET_NAME="backroot8-milestone1.6v3.iso"
RELEASE_TAG="${RELEASE_TAG:-v8-milestone1.6v3}"
REPO="${GITHUB_REPO:-lasangainc/os-backroot}"

log() { echo "[publish-m1.6v3] $*"; }
...
export BACKROOT8_VERSION=8-milestone1.6v3
...
NOTES="$ROOT/RELEASE-v8-milestone1.6v3-notes.md"
if [[ ! -f "$NOTES" ]]; then
    log "Missing $NOTES"
    exit 1
fi
...
if ! sudo -u "${SUDO_USER:-root}" gh release view "$RELEASE_TAG" --repo "$REPO" &>/dev/null; then
    ...
    sudo -u "${SUDO_USER:-root}" gh release create "$RELEASE_TAG" \
        --repo "$REPO" \
        --title "Backroot 8 Milestone 1.6v3" \
        --notes-file "$NOTES"
fi
